...
- `Priority Score` (0-5 scale): PRESENT = 1.0, PARTIALLY_PRESENT = 0.5, summed across configured `STATUS_COLUMNS`
...
- Uses underlying CSV/status column names (e.g., `"Host Species Status"`), while UI headers drop the `" Status"` suffix
...
priority_score <- function(row) {
  ...
  score  # Returns 0-5 with current STATUS_COLUMNS
}
```
...
**Combined Filtering Strategy**: Start with `Priority Score ≥ 4` AND `has_differential_abundance = TRUE` to identify high-quality curatable candidates, then refine thresholds using exploratory analysis on realistic PubMed samples.
...
- **Identify systematic prediction errors** (e.g., host/body site/condition misclassification)
...
├── results/                # Default feedback output dir (`FEEDBACK_DIR`), separate from input data
...
**Priority Score**: Computed metric (0-5 scale) indicating completeness of metadata. Higher = more likely curatable.
