...
::: {#feedback-form .card .shadow-sm .w-100}
...
::: {.feedback-top-col}
**Curator ID / initials**  
<input type="text" id="curator_id" class="form-control feedback-input" placeholder="e.g. Ronald Ouma">
:::
...
if (nrow(df) > 0 && "PMID" %in% names(df)) {
  ...
  cat('<select id="quick-select-pmid" class="form-select feedback-input"><option value="">— Select a PMID —</option>')
  for (i in seq_len(nrow(opts))) {
    pm <- opts$PMID[i]
    lbl <- if ("Title" %in% names(opts)) paste0(pm, " — ", substr(opts$Title[i], 1, 60), if (nchar(as.character(opts$Title[i])) > 60) "…" else "") else as.character(pm)
...
<select id="overall_verdict" class="form-select feedback-input">
...
<input type="text" id="bioanalyzer_version" class="form-control feedback-input" readonly style="background-color: var(--bs-light, #f8f9fa);" title="Version is set at build time.">
:::
...
::: {.mb-3 .mt-3}
**Comment (optional)**  
<textarea id="comment" class="form-control" rows="3" placeholder="Evidence, edge case, missing field, false positive reason..." style="width: 100%;"></textarea>
:::
...
<div class="feedback-field">
  ...
  <div class="feedback-selects">
    <div class="feedback-select-group">
      <small>Curator TRUE label for %s:</small>
      <select id="true_%s" class="form-select form-select-sm">
        <option value="Not reviewed">Not reviewed</option>
    ...
    <div class="feedback-select-group">
      ...
      <select id="col_fb_%s" class="form-select form-select-sm">
        <option value="Not reviewed">Not reviewed</option>
...
Reviewers will be looking into your submitted issue to evaluate the results
:::
:::
:::
...
/* ============================================================
   ...
   Forces the feedback form to break out of Quarto's default
   narrow centered content column, matching the data table.
   ============================================================ */

/* Make the column-screen wrapper and feedback card stretch edge-to-edge */
.column-screen #feedback-form,
.quarto-column-screen #feedback-form {
  width: 100%;
  max-width: 100%;
}
...
/* ============================================================
   TOP METADATA GRID
   Lays out Curator ID, Quick Select, PMID, Title, Verdict,
   and Version in a responsive multi-column grid.
   ============================================================ */
...
/* Inputs inside the top grid stretch to fill their cell */
.feedback-input {
  width: 100% !important;
  max-width: 100% !important;
  box-sizing: border-box;
...
/* ============================================================
   FIELD-BY-FIELD VALIDATION GRID
...
/* ============================================================
   INDIVIDUAL FEEDBACK FIELD CARD
   ============================================================ */
...
.feedback-field .bioanalyzer-pred-line {
...
/* Two selects side by side inside each field card */
.feedback-selects {
  display: grid;
...
.feedback-select-group {
  display: flex;
  flex-direction: column;
  gap: 0.2rem;
}

.feedback-select-group select {
  width: 100%;
  max-width: 100%;
  box-sizing: border-box;
}
