...
if (nrow(df) == 0) {
  ...
  # ---- Convert PMID to clickable column ----
  
  if ("PubMed Link" %in% names(show_df)) {
    show_df <- show_df |>
      dplyr::mutate(PMID = paste0('<a href="', `PubMed Link`,
                                  '" target="_blank" rel="noopener">',
                                   PMID, '</a>')) |>
    dplyr::select(-`PubMed Link`)
  }

  # ---- Shorten status column names ----
  
  # Could this be shortened more easily by changing config.R?
  show_df <- show_df |>
    dplyr::rename_with(~ gsub(" Status", "", .x), dplyr::all_of(STATUS_COLUMNS))

  if ("Priority Score" %in% names(show_df)) {
    show_df <- show_df |> dplyr::rename(Priority = `Priority Score`)
  }
  ...
  priority_index <- which(names(show_df) == "Priority")
